...
from template.controls.controllers import MetroMapController
import panel as pn
...
class MetroMap(VisualizationInterface):
    ...
    def _calculate(self, calculating):
        ...
        if len(self._controls.components_int) != 1:
            data = hv.Image(self._main._pipe.data).apply.opts(cmap='Blues',
                                                              color_levels=self._controls.param.water_level,
                                                              width=self._main._width, height=self._main._height,
                                                              xlim=self._main._xlim, ylim=self._main._ylim)
        else:
    ...
    def _doSnapping(self, point, position, direction, line):
        ...
        while step + direction >= 0 and step + direction < len(line):
            step = step + direction
            point = self._get_closest_point(point, line[step])
    ...
    def _get_closest_point(self, pt1, pt2):
